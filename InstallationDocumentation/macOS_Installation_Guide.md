...
Search for listen_addresses and change to:

text
listen_addresses = '*'
Search for port and ensure it says:

text
port = 5432
Scroll to the bottom (to the "CUSTOMIZED OPTIONS" section) and add:
...
Linux	macOS	Purpose
apt-get	brew	Package manager
useradd/groupadd	Not needed	User management (run as your user)
systemd/tmpfiles	Not needed	Service management
~/.bashrc	~/.zshrc	Shell configuration
LD_LIBRARY_PATH	DYLD_LIBRARY_PATH	Library path variable
build-essential	xcode-select	Development tools
--with-uuid=ossp	--with-uuid=e2fs	UUID library
/var/run/openhalo	/tmp	Socket directory
Support
